...
export class AppProfileSetup extends LitElement {
  ...
  private async _fetchBackendProfiles() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        this.profilesError = 'Failed to load profiles';
      }
    } catch {
      this.profilesLoadingState = 'error';
      this.profilesError = 'Cannot reach backend';
    }
  }
  ...
  private async _checkInstance() {
    ...
    this.syncCheckMessage = 'Checking…';
    ...
    try {
      ...
      if (res.ok) {
        ...
        this.syncCheckMessage = `Found: "${data.profile?.name ?? this.syncId}"`;
      } else {
        this.syncCheckState = 'notfound';
        this.syncCheckMessage = 'Instance not found on server.';
      }
    } catch {
      this.syncCheckState = 'error';
      this.syncCheckMessage = 'Cannot reach backend.';
    }
  }
  ...
  private async _doCreate() {
    ...
    if (!name) {
      this.createError = 'Please enter a profile name.';
      return;
    }
    ...
    try {
      ...
      this.createError = err?.message ?? 'Failed to create profile.';
    }
  }
      ...
      <div class="modal-overlay">
        <div class="modal-card">
          ...
          <div class="modal-header">
            ...
            <h1>Welcome to Overlay Framework</h1>
            <p>This device has no profile yet. Sync with an existing instance or create a new one.</p>
          </div>
          ...
          <div class="panels">
            ...
            <div class="panel">
              <div class="panel-title">
                ...
                <div>
                  <h2>Sync existing instance</h2>
                  <p>Link to a profile you already have</p>
                </div>
              </div>
              ...
              ` : html `
                ${this.profilesLoadingState === 'error' ? html `
                  <div class="alert-warning">
                    Could not load existing profiles. You can enter an ID manually below.
                  </div>
                ` : html `
                  ${this.profilesLoadingState === 'loading' ? html `
                    <div class="check-status loading">
                      <div class="spinner"></div>
                      <span>Looking for existing profiles...</span>
                    </div>
                  ` : ''}
                `}
              `}
              ...
              <div class="form-group">
                <label class="form-label">${this.backendProfiles.length > 0 ? 'Or enter Instance ID manually' : 'Instance ID'}</label>
                <input
                  id="sync-id-input"
                  class="form-input ${this.syncCheckState === 'notfound' || this.syncCheckState === 'error' ? 'error' : ''}"
                  type="text"
                  placeholder="overlay-1234567-abc123"
                  .value="${this.syncId}"
              ...
              ${this.syncCheckState === 'idle' || this.syncCheckState === 'notfound' || this.syncCheckState === 'error'
                ? html`
                    ...
                    >
                      ${this.syncCheckState as string === 'checking'
                        ? html`<div class="spinner"></div> Checking…`
                        : html`
                ...
                : html`
                    ...
                    >
                      ${this.syncSubmitState === 'loading'
                        ? html`<div class="spinner"></div> Syncing data…`
                        : html`
                            <svg style="width:1rem;height:1rem" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="panel">
              <div class="panel-title">
                ...
                <div>
                  <h2>Start fresh</h2>
                  <p>Create a new profile for this device</p>
                </div>
              </div>
              ...
              >
                ${this.createSubmitState === 'loading'
                  ? html`<div class="spinner"></div> Creating…`
                  : html`
                      <svg style="width:1rem;height:1rem" fill="none" viewBox="0 0 24 24" stroke="currentColor">
