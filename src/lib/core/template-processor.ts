...
export function processTemplate(
  ...
  if (eventData) {
    for (const [key, value] of Object.entries(eventData)) {
      const highlightedValue = `<span style="color: ${highlightColor}; font-weight: bold;">${value}</span>`;
  ...
  processedMessage = processedMessage.replace(/\{(\w+)\}/g, `<span style="color: ${highlightColor}; font-weight: bold;">$1</span>`);
